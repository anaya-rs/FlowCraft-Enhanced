...
const AIAnalysisPanel: React.FC<AIAnalysisPanelProps> = ({ summary, keyValues, confidence, entities }) => {
  return (
    <div className="glass-card-dark p-6 w-full max-w-lg mx-auto animate-fade-in">
      <h2 className="text-xl font-bold mb-4 bg-gradient-to-r from-primary to-primary-light bg-clip-text text-transparent">AI Analysis</h2>
      <div className="mb-4">
        <div className="text-sm text-muted-foreground mb-1">Summary</div>
        <div className="text-white text-base font-medium mb-2">{summary}</div>
      ...
      <div className="mb-4">
        <div className="text-sm text-muted-foreground mb-1">Key-Value Pairs</div>
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(keyValues).map(([k, v]) => (
            <div key={k} className="flex flex-col bg-white/5 rounded p-2">
              ...
              <span className="text-xs text-primary">{Math.round(v.confidence * 100)}%</span>
            </div>
          ))}
        </div>
      </div>
      {entities && entities.length > 0 && (
        <div className="mb-2">
          ...
          <div className="flex flex-wrap gap-2">
            {entities.map((e, i) => (
              <span key={i} className="px-2 py-1 rounded bg-gradient-orange text-xs text-white font-medium animate-fade-in">
