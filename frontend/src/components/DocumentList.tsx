  ...
  const handleDeleteDocument = (documentId: string) => {
    ...
    if (savedDocuments) {
      try {
        ...
        // Refresh the document list by calling the parent's refresh function
        if (onRefresh) {
          onRefresh()
        }
      } catch (error) {
      ...
      <div className="document-grid">
        {documents.map((doc, index) => (
          ...
          >
            ...
            {/* Document Info */}
            <div className="mb-4">
              <h3 className={`${commonTypography.subsectionTitle} text-white mb-2 line-clamp-2 font-sans`}>
                {doc.originalFilename}
              </h3>
              
              <div className={`space-y-2 ${commonTypography.caption} text-gray-400`}>
                <div className="flex items-center space-x-2">
            ...
            {doc.aiSummary && (
              <div className="mb-4 p-3 bg-glass-white/5 rounded-lg">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-primary-400 font-sans">AI Summary</span>
                </div>
            ...
            {doc.keyInsights && doc.keyInsights.length > 0 && (
              <div className="mb-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-green-400 font-sans">Key Insights</span>
                </div>
                  ...
                  {doc.keyInsights.slice(0, 3).map((insight, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-gray-400 font-sans">{insight.label}:</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-white font-medium font-sans">{insight.value}</span>
    ...
    <div className="glass-card-dark overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-glass-white/5">
            <tr>
              <th className={`px-6 py-3 text-left ${commonTypography.caption} font-medium text-gray-400 uppercase tracking-wider`}>
                Document
              </th>
              <th className={`px-6 py-3 text-left ${commonTypography.caption} font-medium text-gray-400 uppercase tracking-wider`}>
                AI Summary
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className={`${commonTypography.bodyText} font-medium text-white`}>
                        {doc.originalFilename}
                      </div>
                      <div className={`${commonTypography.bodyText} text-gray-400`}>
                        {doc.documentType}
                        {doc.isHandwritten && (
                          <span className={`ml-2 px-1.5 py-0.5 bg-purple-500/20 text-purple-400 ${commonTypography.caption} rounded`}>
                            Handwritten
                          </span>
                        )}
                      </div>
      ...
      {documents.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className={`${commonTypography.subsectionTitle} text-white mb-2`}>No documents found</h3>
          <p className={`${commonTypography.bodyText} text-gray-400`}>Upload your first document to get started</p>
        </div>
      )}
    </div>
  )
